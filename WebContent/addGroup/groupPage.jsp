		...
		<div class="bg-container"
			...
			<div class="header-container">
				...
				<ul class="category-container">
					<li>
						<c:choose>
							<c:when test = "${ group.bcategorycode == 1 }">
								<c:set var="bcategoryKR" value="영어" />
								</c:when>
							<c:when test = "${ group.bcategorycode == 2 }">
								<c:set var="bcategoryKR" value="외국어" />
								</c:when>
							<c:when test = "${ group.bcategorycode == 3 }">
								<c:set var="bcategoryKR" value="취업" />
								</c:when>
							<c:when test = "${ group.bcategorycode == 4 }">
								<c:set var="bcategoryKR" value="국가고시&공무원" />
								</c:when>
							<c:when test = "${ group.bcategorycode == 5 }">
								<c:set var="bcategoryKR" value="기타" />
								</c:when>
						</c:choose>
						<div class="category">${ bcategoryKR }</div>
						<div class="category">${ group.scategorycode }</div>
					</li>
				</ul>
				<!-- 스터디그룹 타이틀 -->
				<div class="study-title">${group.groupName }</div>
				<!--  그룹장, 등록일, 찜하기, 신청하기버튼을 위한 container -->
				<ul class="user-container">
					<li>
						<div class="user-info">${group.id }</div>
						<div class="regdate">
							등록일 <span class="number">${group.createDate }</span>
						</div>
					</li>
					<li class="pull-right hidden-xs">
						<div class="main-button-container">
							...
							<button class="main-button"
								onclick="javascript:window.location='/semi2/addGroup/applyPro.do?num=${group.groupNum}'">
				...
				<div class="shown-xs main-button-container">
					...
					<button class="main-button" onclick="javascript:window.location='/semi2/addGroup/applyPro.do?num=${group.groupNum}'">
						<em class="fa fa-pencil-square-o"></em> 신청하기
					</button>
				</div>
			</div>
				...
				<li>
					...
					<div class="contents-wrap">
						...
						<div class="custom-table first">
							...
							<div class="custom-table-cell">${group.groupGoal }</div>
						</div> 
						<div class="custom-table">
							<div class="custom-table-cell-title">모집 대상</div>
							<div class="custom-table-cell">${group.target }</div>
						</div>
						
						<div class="custom-table">
							<div class="custom-table-cell-title">모집 인원</div>
							<div class="custom-table-cell number">${group.maxMember }</div>
						</div>
						<div class="custom-table">
							<div class="custom-table-cell-title">회비</div>
							<div class="custom-table-cell date">${group.money}원</div>
						</div>
						<div class="custom-table">
							<div class="custom-table-cell-title">모집 기간</div>
							<div class="custom-table-cell date">${group.limitDate }</div>
						</div>
						<div class="custom-table">
						...
						<div class="custom-table">
							...
							<div class="custom-table-call">
								<div class="col-xs-9" style="margin-bottom:30px;">
									<span class="location main-location"><b>${group.location }</b></span>
								</div>
...
${group.introduce }
			...
			<ul class="contents-list">
				<li>
					<div class="contents-title">신청자 정보</div>
				 <div class="table-responsive">
    <!--  DB에서 불러온 정보를 나열 -->
    <c:set var="DB" value="${member}" />
    <c:if test="${DB eq null}">
        	  <table class="table">
    <thead>
      <tr>
        <th>이름</th>
    ...
    <tr>
   	<td colspan="7" style="text-align:center;">신청자가 없습니다.</td>
   	</tr>
   	</table>
    </c:if>
    <c:if test="${DB ne null}">
    	<c:if test="${check == 1 }">
    	  <table class="table">
    <thead>
      <tr>
